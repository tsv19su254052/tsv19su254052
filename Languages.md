...
Библиотека [tkvue](https://pypi.org/project/tkvue) [💬](https://gitlab.com/ikus-soft/tkvue) :
 ...
 - Своеобразна тем, что макет (эскиз) в виде XML-ной разметки можно вынести из исходника во внешний файл или в XML-ное поле SQL-ной базы данных и на исполнении парсить его оттуда как SAX - некая доработка [XUL](https://ru.wikipedia.org/wiki/XUL).
 - В принципе можно понарисовать макетов на все случаи жизни и еще написать простое и нетрудоемкое WEB-приложение.
 - Но дизайнера для рисования макетов (эскизов) графической оболочки пока не обнаружено.
 - Поэтому макеты придется писать вручную и большую сложность поднять тяжело. 
...
Есть бесплатная и есть коммерческая (нужна для [Django](https://en.wikipedia.org/wiki/Django_(web_framework)) и автоматического тестирования, коннектится в серверам СУБД и кое-как выполняет автономные запросы)
