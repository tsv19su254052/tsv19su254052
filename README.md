...
Для простых прикладных задач вполне подходят скрипты, работающие в командной строке с ключами и путями. Для одновременного обозревания нескольких параметров в разных прикладных задачах разрабатываются и применяются графические оболочки (диалоги и окна), в том числе для работы с данными из СУБД. Если нужно что-то явить миру, поверх прикладной компоненты делается ВЭБ-сайт (в объемы данных репозиториев не входит, а выполняется отдельно).
