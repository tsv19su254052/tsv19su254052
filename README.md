...
Проекты со вставками по подсказкам со `stackoverflow` и с `ChatGPT` хранятся на сетевой папке, полностью функциональны только внутри инфраструктуры и могут запускаться на исполнение следующими способами:
 - Из внешней сети или в локальной подсетке по RDP на терминальном сервере. Один набор интерпретаторов и их библиотек для всех пользователей. Глюков быть не должно.
