...
Репозитории индувидуальной разработки со вставками по подсказкам из `stackoverflow` и со вставками, сгенерированными с помощью `ChatGPT`, здесь и в организациях являются так сказать неполными **бэкапами с оригиналов проектов** (только отслеживаемые на `Git`-е файлы проекта).
