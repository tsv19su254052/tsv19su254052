...
С появлением вычислительной техники в 1990-х многие императивные и приближенные методы в прикладной математике с номограммами потеряли свою практическую пользу. Стало возможно посчитать аналитические формулы напрямую, используя конечно-разностные формы их записи. Посчитали многие задачи в том числе различные оптимизации, которые ждали своего решения еще с 1960-х годов. Для удобства восприятия решение как правило стремились получить в виде экстремума градиента на поверхности или на облаке точек, подбирая диапазоны итерации вручную или методом последовательного приближения.
...
Для простой прикладной задачи вполне подходит скрипт, работающий в командной строке с ключами и с путями. Для одновременного обозревания нескольких параметров в том числе для работы с данными из СУБД в прикладной задаче разрабатывается и применяется графическая оболочка (окно с диалогами или диалог с диалогами). Если нужно что-то явить миру, поверх прикладной компоненты (серверной или клиентской) делается ВЭБ-сайт (выполняется отдельно, в объем данных репозиториев может входить в виде подмодуля как внешняя разработка).
...
API-шки в своем широком понимании применяются для обеспечения работы модели "Клиент - Сервер" локально или по локальным сетям для повторного применения пользовательских типов данных и методов работы с ними в исходных текстах (функции, статические и динамические библиотеки, интерфейсы, технологии СОМ и DCOM) разной степени гибкости
