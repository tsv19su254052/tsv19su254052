...
Для `XML`-ных структур данных применяются:
 ...
 - [xProc](https://en.wikipedia.org/wiki/XProc) [💬](https://spec.xproc.org/3.1/xproc) [💬](https://www.w3.org/TR/xproc/)- сложное последовательное преобразование возможно в несколько шагов с условными переходами в несколько путей с помощью вышеперечисленных (например, сборка или соединение нескольких внешних файлов (ресурсов) через `xLink` или `xInclude`, проверка через схемы `XSD`, преобразование `XSLT`), <!-- todo Там внутри функциональной достаточно сложная процедурная составляющая, надо поискать аналог в библиотеках на Python-е -->
...
<!-- 
todo Тут было бы неплохо дать графическую часть что во что входит, применяется и где используются одни и те же типы данных. Нужна видимо полноценная среда разработки, только подсветки синаксиса наверное будет мало
-->
