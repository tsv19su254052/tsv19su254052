...
todo Вообще все-таки надо выяснить:
...
Для `XML`-ных структур данных применяются:
 ...
 - [XSLT](https://en.wikipedia.org/wiki/XSLT), [eXSLT](https://en.wikipedia.org/wiki/EXSLT) - преобразование в один ход с путями на `xPath`,
 - [Saxon](https://www.saxonica.com/about/about.xml) - интерпретатор `XSLT` в том числе,
 - [xProc](https://en.wikipedia.org/wiki/XProc) [💬](https://spec.xproc.org/3.1/xproc) [💬](https://www.w3.org/TR/xproc/)- сложное последовательное преобразование возможно в несколько шагов с условными переходами в несколько путей с помощью вышеперечисленных (например, привязка через `xLink` и вставка через `xInclude` нескольких внешних файлов (ресурсов), проверка через схемы `XSD`, `Relax NG`, `Schematron`, условия на `xQuery`, преобразование через `XSLT`),
...
todo:
 - Там внутри функциональной части `xProc` достаточно сложная процедурная составляющая, надо поискать, возможно есть аналог в библиотеках на Python-е.
