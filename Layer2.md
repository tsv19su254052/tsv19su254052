...
Если нужно что-то для нечастого или начального использования вручную с ключами, с путями и с последовательным вводом-выводом или для вторичных вызовов как функция, [API](https://en.wikipedia.org/wiki/API)-шка , то пишем [скрипт](https://timeweb.com/ru/community/articles/chto-takoe-skript):
 ...
 - на `SQL` со вставками на `XPath & XQuery`, вставляем его в [хранимую процедуру](https://en.wikipedia.org/wiki/Stored_procedure) базы данных и вызываем из `Python`-а с передачей параметров на вход и выход.
...
Четкой разницы между тем и этим нету, потому что удобнее применять комбинированный подход по ситуации, используя их оба. В настоящее время преимущество первого - когда вторым никак ни сделать, а второго - только экономия ОЗУ и экономия на промежуточных переменных. Само собой изменяемую запись в базе данных надо заблокировать транзакцией, которая соответсвенно должна отработать максимально быстро.
