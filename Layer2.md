...
Есть интересная библиотека [SQLAlchemy](https://docs.sqlalchemy.org/en/14/dialects/mssql.html#module-sqlalchemy.dialects.mssql.pyodbc) как слой абстракции в режиме [Core](https://docs.sqlalchemy.org/en/20/core) или в режиме [ORM](https://docs.sqlalchemy.org/en/20/orm/), но она пока умеет не очень сложные вещи. В начале ее разработки была надежда, что она даст функционал, позволяющий перенести вычислительную нагрузку с `SQL` на например `Python`, так как среды разработки на `Python`-е видят на `SQL` и на `xPath & xQuery` внутри `SQL`, как текст, не понимают их синтаксис (нет автодополнения, подсказок, не понимают их структуру и типы данных) и не дают что-то в них выборочно выполнить или отладить.
