...
[SQL Server](https://en.wikipedia.org/wiki/Microsoft_SQL_Server) при возрастании нагрузки с клиентов откидывает запросы по взаимоблокировке. Обертываем запрос в обработку исключения в цикле попыток с нарастающей задержкой, но попытки соответственно встают на паузы и сервер недогружается. Снять попытки с паузы можно обратными вызовами от Service Brocker, но там много ручной работы.
...
При использовании хранимых процедур на `SQL` (с целью передачи части вычислительной нагрузки с клиента на сервер СУБД) в преобразовании типов данных по цепочке `Python <-> SQL <-> XML <-> XPath & XQuery` (cм. пространства имен [XDM](https://en.wikipedia.org/wiki/XQuery_and_XPath_Data_Model)) есть [тонкости 💬](https://en.wikipedia.org/wiki/Object%E2%80%93relational_impedance_mismatch "Вступительная часть по освещению данной темы"), так как те аскетичнее этих и не перекрываются полностью. Для работы с `XML`-ными полями средствами `Python`-а есть интересные возможности у библиотек:
