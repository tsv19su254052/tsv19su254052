...
Вернувшись к началу и суммируя, хочется отметить, что вообще-то суть не в разновидности `SQL`-ная база ли или нет. А в самом принципе. Есть только некоторая часть данных, которые можно выразить в виде таблиц (если так будет удобнее и как бы понятнее). Исторически сложилось, что для таких структур придумали табличные базы данных, которые назвали `SQL`-ными, потому что язык запросов к ним называется `SQL`. Таким образом, если разобраться, происходит сбивание с толку в два хода.
